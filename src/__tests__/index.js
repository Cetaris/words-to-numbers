...
const wtn = require('../../index').wordsToNumbers;
const { it } = global;
...
describe('Words to number', function () {
  describe('Default test cases', function () {
    ...
    it('twenty thousand five hundred and sixty nine', () => {
      expect(wtn('twenty thousand five hundred and sixty nine')).to.equal(
        20569
      );
    });
    ...
    it('there were twenty-thousand, five-hundred and sixty-nine X in the five quintillion Y', () => {
      expect(
        wtn(
          'there were twenty-thousand, five-hundred and sixty-nine X in the five quintillion Y'
        )
      ).to.equal('there were 20569 X in the 5000000000000000000 Y');
    });
    ...
    it('three point one four one five nine two six', () => {
      expect(wtn('three point one four one five nine two six')).to.equal(
        3.1415926
      );
    });
    ...
    it('Fifty People, One Question: Brooklyn', () => {
      expect(wtn('Fifty People, One Question: Brooklyn')).to.eq(
    ...
    it('Ten Thousand Pictures of You', () => {
      expect(wtn('Ten Thousand Pictures of You')).to.eq(
        '10000 Pictures of You'
      );
    });

    it('nineteen eighty four', () => {
      expect(wtn('nineteen eighty four', { impliedHundreds: true })).to.eq(
        1984
      );
    });
    ...
    it('fifty sixty three thousand', () => {
      expect(
        wtn('fifty sixty three thousand', { impliedHundreds: true })
      ).to.eq(5063000);
    });
    ...
    it('I have zero apples and four oranges', () => {
      expect(wtn('I have zero apples and four oranges')).to.eq(
        'I have 0 apples and 4 oranges'
      );
    });
  ...
  const removeSpaces = (stringOrNumber) =>
    stringOrNumber.toString().replace(/\s/g, '');

  describe('FAR test cases', function () {
    words = [
      // Input format Integer:
      { input: 'one thousand four', expect: 1004 },
      { input: 'one thousand and four', expect: 1004 },
      { input: 'one zero zero four', expect: 1004 },
      { input: 'one oh oh four', expect: 1004 },
      { input: 'one comma zero zero four', expect: 1004 },
      { input: 'one double-oh four', expect: 1004 },
      // Input format Real:
      { input: 'one thousand four point one five', expect: 1004.15 },
      { input: 'one thousand four point fifteen', expect: 1004.15 },
      { input: 'one thousand four dot one five', expect: 1004.15 },
      { input: 'one thousand four dot fifteen', expect: 1004.15 },
      { input: 'one thousand four decimal one five', expect: 1004.15 },
      { input: 'one thousand four decimal fifteen', expect: 1004.15 },
      { input: 'seventeen fifty', expect: 1750 },
      { input: 'eighteen twenty-six', expect: 1826 },
      { input: 'nineteen eighty-four', expect: 1984 },
      { input: 'twenty seventeen', expect: 2017 },
      { input: 'fourteen hundred', expect: 1400 },
      { input: 'seventeen hundred', expect: 1700 },
      { input: 'two thousand', expect: 2000 },
      { input: 'fourteen oh one', expect: 1401 },
      { input: 'seventeen oh one', expect: 1701 },
      { input: 'two thousand and one', expect: 2001 },

      { input: 'ten point five', expect: 10.5 },
      {
        input: 'three point one four one five nine two six',
        expect: 3.1415926,
      },
      { input: 'first', expect: 1 },
      { input: 'second', expect: 2 },
      { input: 'third', expect: 3 },
      { input: 'fourteenth', expect: 14 },
      { input: 'twenty fifth', expect: 25 },
      { input: 'thirty fourth', expect: 34 },
      { input: 'forty seventh', expect: 47 },
      { input: 'fifty third', expect: 53 },
      { input: 'sixtieth', expect: 60 },
      { input: 'seventy second', expect: 72 },
      { input: 'eighty ninth', expect: 89 },
      { input: 'ninety sixth', expect: 96 },
      { input: 'one hundred and eighth', expect: 108 },
      { input: 'one hundred and tenth', expect: 110 },
      { input: 'one hundred and ninety ninth', expect: 199 },
      { input: 'one hundred', expect: 100 },
      ...
      // "fifty sixty three",
      // "fifty sixty",
      // "fifty sixty three thousand",
      // "one hundred thousand",
    ];

    words.forEach((testCase) => {
      it(testCase.input, () => {
        expect(removeSpaces(wtn(testCase.input))).to.eq(
          removeSpaces(testCase.expect)
        );
      });
    });
  });
